// returns clamp(0, floor(log2(x/y)), 31) where x and y are integers. not prone to floating point errors.
const floorLog2=(x,y=1)=>{
	let n=0,w=y*65536;
	if(x>=w)y=w,n+=16;w=y*256;
	if(x>=w)y=w,n+=8;w=y*16;
	if(x>=w)y=w,n+=4;w=y*4;
	if(x>=w)y=w,n+=2;
	x<y*2||n++;
	return n
},
// returns clamp(1, ceil(log2(x/y)), 32) where x and y are integers. not prone to floating point errors.
ceilLog2=(x,y=1)=>{
	let n=1,w=y*65536;
	if(x>w)y=w,n+=16;w=y*256;
	if(x>w)y=w,n+=8;w=y*16;
...
// returns [m, e, m * 10^e] where (m-1)*10^e<v<=m*10^e, m<100 and m mod 10!=0. therefore `${m}e${e}` is an upper bound approximation with ~2 significant digits.
approximateWithTwoSigDigits=v=>{
	if(v<=0)return[0,0,0]; // special case
	for(var exp=0,tens=100;v>=tens;tens*=10)++exp;
	let mant=Math.ceil(v/(tens/=100));
	mant%10||(mant/=10,++exp,tens*=10); // 60e6 -> 6e7
	return[mant,exp,mant*tens]
},
// Node.js 14 doesn't have a global performance object.
getPerformanceObject=async()=>globalThis.performance||(await import("perf_hooks")).performance;
//------------------------------------------------------------------------------
class ResourcePool{
	constructor(){
	...
	allocate(parent,size){
		const a=this.arrayBuffers.get(size);
		return a&&a.pop()||new ArrayBuffer(size)
	}
	//FinalizationRegistry is also possible,but GC couldn't keep up with the memory usage
	release(A){
		let n=A.byteLength,a=this.arrayBuffers.get(n);
		a||this.arrayBuffers.set(n,a=[]);
		a.push(A)
	}
	wasmDefaultModelRunner(contextItemShift){
		var a=this.wasmRunners.get(contextItemShift);a||this.wasmRunners.set(contextItemShift,a=makeDefaultModelRunner(contextItemShift));
		return a
	}
}
const newUintArray=(pool,parent,nbits,length)=>{
	if(nbits<9)return new Uint8Array(pool?pool.allocate(parent,length):length);
	if(nbits<17)return new Uint16Array(pool?pool.allocate(parent,length*2):length);
	if(nbits<33)return new Uint32Array(pool?pool.allocate(parent,length*4):length);
	throw'newUintArray: nbits is too large'
},
//------------------------------------------------------------------------------
// this is slightly configurable (ryg_rans equivalent would be 31), but we already have too many parameters.
ANS_BITS=28;

// roughly based on https://github.com/rygorous/ryg_rans/blob/master/rans_byte.h
class AnsEncoder{
	constructor({outBits,precision}){
		this.precision=precision;// all input frequencies are assumed to be scaled by 2^precision
		this.outBits=outBits;// the number of output bits, or -(the number of output symbols) if negative
		this.input=[]// the bits and corresponding frequencies to be written. rANS decoder and encoder works in the reverse. therefore we buffer bits and encode them in the reverse, so that the decoder will decode bits in the correct order
	}
	writeBit(bit,predictedFreq){
		if(0!==bit&&1!==bit)throw new Error("AnsEncoder.writeBit: bad bit");
		if(predictedFreq!==~~predictedFreq||predictedFreq<0||predictedFreq>=1<<this.precision)throw new Error("AnsEncoder.writeBit: bad predictedFreq");
		if(this.finished)throw new Error("AnsEncoder.writeBit: already finished");
		this.input.push(predictedFreq<<1|bit)
	}
	finish(){
		if(this.finished)throw new Error("AnsEncoder.writeBit: already finished");
		this.finished=true;
		const buf=[],outSymbols=this.outBits<0?-this.outBits:1<<this.outBits,probScale=this.precision+1,stateShift=ANS_BITS-ceilLog2(outSymbols)-probScale,p0=1<<probScale;
		let state=1<<ANS_BITS-ceilLog2(outSymbols),o=0;
		for(const f of this.input.reverse()){
			...
			let b=~f&1,p=f|1,start=p*b,size=b?p0-p:p;
			// renormalize
			for(p=size*outSymbols<<stateShift;state>=p;state=state/outSymbols|0)buf[o++]=state%outSymbols;
			state=(state/size<<probScale)+state%size+start;// add the bit to the state
			if(2147483647<state)throw new Error("ANSEncoder.finish: state overflow")
		}
		for(;state;state=state/outSymbols|0)buf[o++]=state%outSymbols;
		return buf.reverse()
	}
}
class AnsDecoder{
	...
	readBit(predictedFreq){
		const prob=predictedFreq<<1|1,probScale=this.precision+1,rem=this.state&(1<<probScale)-1,bit=0|rem<prob;
		let start,size;
		if(bit)start=0,size=prob;
		else start=prob,size=(1<<probScale)-prob;
		// renormalize
		for(this.state=size*(this.state>>probScale)+rem-start;this.state<this.renormLimit;this.state+=this.buf[this.offset++]%this.outSymbols){
			if(this.offset>=this.buf.length)throw new Error('AnsDecoder.readBit: out of buffer bounds');
			this.state*=this.outSymbols
		}return bit
	}
}
//------------------------------------------------------------------------------
class DirectContextModel{
	constructor({inBits,contextBits,precision,modelMaxCount,modelRecipBaseCount,arrayBufferPool,resourcePool}){
		this.bitContext=1;this.inBits=inBits;
		...
		if(this.resourcePool=resourcePool||arrayBufferPool)
			// we need to initialize the array since it may have been already used, but UintXXArray.fill is comparatively slow, less than 5 GB/s even in fastest browsers. we instead use more memory to confirm that each bit of context has been initialized. the final excess element is the maximum mark in use(this kind of size is not used elsewhere, so we can safely reuse that) we choose a new mark to mark initialized elements *in this instance*. if the mark reaches 255 we reset the entire array and start over. this scheme effectively reduces the number of fill calls by a factor of 510.
			this.confirmations=newUintArray(this.resourcePool,this,8,1<<contextBits|1),
			this.mark=this.confirmations[1<<contextBits]+1,
			this.mark^256||(this.mark=1,this.confirmations.fill(0)),
			this.confirmations[1<<contextBits]=this.mark;
		else this.predictions.fill(1<<precision-1)//,this.counts.fill(0) // we don't really need this
	}
	predict(context=0){
		context=context+this.bitContext&(1<<this.contextBits)-1;
		if(this.confirmations&&this.confirmations[context]!==this.mark)
			this.confirmations[context]=this.mark,this.predictions[context]=1<<this.precision-1,this.counts[context]=0;
		return this.predictions[context]
	}
	update(actualBit,context=0){
		if(actualBit!==0&&actualBit!==1)throw new Error('DirectContextModel.update: bad actualBit');
		this.counts[context=context+this.bitContext&(1<<this.contextBits)-1]<this.modelMaxCount&&++this.counts[context];
/*
		...
		therefore P' >= 0
*/
		this.predictions[context]+=((actualBit<<this.precision)-this.predictions[context]<<29-this.precision)/(this.counts[context]+this.modelBaseCount)>>29-this.precision;
		this.bitContext+=this.bitContext+actualBit
	}
	flushByte(){this.bitContext=1}
	release(){
		if(this.resourcePool){
			...
			this.predictions=this.counts=this.confirmations=null
		}
	}
}
class SparseContextModel extends DirectContextModel{
	constructor(options){
		super(options);
		for(var s=this.selector=options.sparseSelector,b=this.recentBytes=[],i=this.sparseContext=0;s>>i;)b[i++]=0
	}
	predict(context=0){return super.predict(this.sparseContext+context)}
	update(actualBit,context=0){super.update(actualBit,this.sparseContext+context)}
...
class LogisticMixModel{
	constructor(models,{recipLearningRate,precision}){
		this.models=models;
		this.precision=precision;
		this.recipLearningRate=recipLearningRate;
		this.mixedProb=0;
		this.stretchedProbs=new Float64Array(this.mn=models.length);
		this.weights=new Float64Array(this.mn)
	}
	predict(context=0){
		for(var i=this.mn,total=0;i;){
			const prob=this.models[--i].predict(context)*2+1,stretchedProb=this.stretchedProbs[i]=Math.log(prob/((2<<this.precision)-prob));
			total-=this.weights[i]*stretchedProb
		}
		// since CM is the last model and predictions are not stored, we can just compute the external probability directly.
		this.mixedProb=i=1|((2<<this.precision)-1)/(1+Math.exp(total));
		// this adjustment can be combined and elided in the optimized decompressor
		return i>>1
	}
	update(actualBit,context=0){
		for(let i=this.mn,mixedProb=this.mixedProb/(2<<this.precision);i;this.weights[i]+=this.stretchedProbs[i]/this.recipLearningRate*(actualBit-mixedProb))
	...
	release(){for(const model of this.models)model.release&&model.release()}
}
//------------------------------------------------------------------------------
class DefaultModel extends LogisticMixModel{
	constructor(options){
		const{inBits,sparseSelectors,modelQuotes}=options;
		super(sparseSelectors.map(sparseSelector=>new SparseContextModel({...options,sparseSelector})),options);
	...
	flushByte(currentByte,inBits){
		super.flushByte(currentByte,inBits);
		if(this.modelQuotes)
			this.quote&&this.quote===currentByte?this.quote=0:!this.quote&&[34,39,96].includes(currentByte)&&this.quotesSeen.add(this.quote=currentByte)
	}
}
//------------------------------------------------------------------------------
const compressWithModel=(input,model,options)=>{
	const l=input.length,encoder=new AnsEncoder(options),{inBits,outBits,precision,preset=[],inputEndsWithByte,calculateByteEntropy}=options;
	if(inputEndsWithByte!==void 0){
		if(!l)
			throw new Error('compressWithModel:inputEndsWithByte given but input is empty');
		if(input[l-1]!==inputEndsWithByte)
			throw new Error('compressWithModel:input does not agree with inputEndsWithByte');
		for(let a=0,n=l-1;a<n;)
			if(input[a++]===inputEndsWithByte)
				throw new Error('compressWithModel:input contains multiple inputEndsWithByte')
	}
	for(let a=0,n=preset.length;a<n;){
		const c=preset[a++];
		for(let i=inBits;i;model.update(c>>--i&1))model.predict();
		model.flushByte(c,inBits)
	}
	const byteProbs=[];
	for(let a=0;a<l;){
		const c=input[a++];
		let bp=calculateByteEntropy?1:0;
		for(let i=inBits;i;){
			const b=c>>--i&1,p=model.predict();
			if(bp)bp*=2*(b?p:(1<<precision)-p)+1;
			encoder.writeBit(b,p);
			model.update(b);
		}
		model.flushByte(c,inBits);
		calculateByteEntropy&&byteProbs.push(bp)
	}
	model.release&&model.release();
	const buf=encoder.finish();

	let byteEntropy=calculateByteEntropy&&byteProbs.map(p=>~precision*-inBits-Math.log2(p));
	const bufLengthInBytes=Math.ceil(buf.length*(outBits<0?Math.log2(-outBits):outBits)/8);
	const inputLength=inputEndsWithByte===undefined?l:-1;//so that the caller can't rely on this
	return{buf,inputLength,bufLengthInBytes,byteEntropy}
};
const compressWithDefaultModel=(input,options)=>{
	//if the model is _exactly_ a DefaultModel and no fancy options are in use,we have a faster implementation using JIT-compiled WebAssembly instances.we only use it when we have a guarantee that the wasm instance can be cached.
	let runDefaultModel;
	const resourcePool=options.resourcePool||options.arrayBufferPool;
	// wasm has bug on sparseSelectors.length>32
	if(resourcePool&&!(options.preset||[]).length&&!options.disableWasm&&!options.calculateByteEntropy&&options.sparseSelectors.length<33&&options.sparseSelectors.every(sel=>sel<0x8000))
		try{runDefaultModel=resourcePool.wasmDefaultModelRunner(getContextItemShift(options))}
		catch(e){}//WebAssembly is probably not supported,fall back to the pure JS impl
	if(runDefaultModel){
		const encoder=new AnsEncoder(options),inputLength=input.length,{ predictions,quotesSeen }=runDefaultModel(input,options),{inBits,outBits}=options;
		for(let a=0,o=0;a<inputLength;)
			for(let i=inBits,c=input[a++];i;)
				encoder.writeBit(c>>--i&1,predictions[o++]);
		const buf=encoder.finish(),bufLengthInBytes=Math.ceil(buf.length*(outBits<0?Math.log2(-outBits):outBits)/8);
		return{buf,inputLength,bufLengthInBytes,quotesSeen}
	}
	const model=new DefaultModel(options),ret=compressWithModel(input,model,options);
	ret.quotesSeen=model.quotesSeen;
	return ret
};
const decompressWithModel=({state,buf,inputLength},model,options)=>{
	const{inBits,preset=[],inputEndsWithByte}=options;
	const decoder=new AnsDecoder({state,buf},options);
	for (let offset=0,l=preset.length;offset<l;model.flushByte(code,inBits))
		for(let i=inBits,code=preset[offset++];i;model.update(code>>--i&1))model.predict();
	const reconstructed=[];
	if(inputEndsWithByte!==void 0){
		let current;
		do{
			current=0;
			for(let i=inBits;i--;){
				const actualBit=decoder.readBit(model.predict());
		...
		for(let offset=inputLength;offset--;){
			let current=0;
			for(let i=inBits;i--;){
				const actualBit=decoder.readBit(model.predict());
				current=current<<1|actualBit;
...
const defaultSparseSelectors=(numContexts=12)=>{
...
const predictionBytesPerContext=options=>8<options.precision?16<options.precision?4:2:1;
...
const contextBitsFromMaxMemory=options=>{
	...
	return contextBits-=actualNumContexts*bytesPerContext>1048576*options.maxMemoryMB
};
...
const TEXT_DECODER_THRESHOLD=65000,DYN_MODEL_QUOTES=1;
...
class Packer{
	constructor(inputs,options={}){
		...
		if(this.options.dynamicModels===void 0)
	...
	static prepareText(inputs){
		let text=inputs.map(input=>input.data).join(''),u=text.length>=TEXT_DECODER_THRESHOLD||text.match(/[^\x00-\xff]/);
		return{utf8:u,text:u?unescape(encodeURIComponent(text)):text}
	}
	static prepareJs(inputs,{dynamicModels,numAbbreviations}){
		//we strongly avoid a token like 'this\'one' because the context model doesn't know about escapes and anything after that would be suboptimally compressed.
		//we can't still avoid something like `foo${`bar`}quux`, where `bar` would be suboptimally compressed, but at least we will return to the normal state at the end.
		...
		modelQuotes=dynamicModels&DYN_MODEL_QUOTES,identFreqs=new Map,inputTokens=[];
			...
			for(const token of jsTokens(input.data)){
				...
				switch(token.type){
					...
					case TYPE_IdentifierName:
						1<token.value.length&&identFreqs.set(token.value,(identFreqs.get(token.value)||0)+1);
						break;
		...
		for(let i=128;i;)
			...
			--i==32||modelQuotes&&[34,39,96].includes(i)||unseenChars.add(String.fromCharCode(i));
		...
		// "commonness" is determined by a heuristic score and not by an output length, since the compressor does perform a sort of deduplication via its context modelling and it's hard to calculate the actual compressed size beforehand.
		// therefore this should be rather thought as reducing the burden of context models(this is also why the full deduplication actually performs worse than no deduplication)
		...
		let output='',prevToken,consecutiveAbbrs=new Set; // a set of two abbreviated chars appearing in a row;
		const NEED_SPACE_BEFORE=1,NEED_SPACE_AFTER=2,needSpace={};
		for(const tokens of inputTokens){
			for(const token of tokens){
				/*
				...
				*/
				...
				if(prevToken&&(
					...
					if(prevToken.abbr)
						...
						else needSpace[prevToken.abbr]|=NEED_SPACE_AFTER;
					else if(token.abbr)
		...
		// this is actually a hard problem, corresponding to finding 2-coloring of given bipartite graph with the biggest difference in partition sizes, which is a set cover problem in disguise.
		...
		for(;;){
			...
			for(const abbrs of consecutiveAbbrs){
				const left=abbrs.charCodeAt(0)<<1,right=abbrs.charCodeAt(1)<<1|1;
	...
	static doPack(preparedText,preparedJs,mainInputAction,options){
		...
		const{buf,inputLength,bufLengthInBytes,quotesSeen}=compressWithDefaultModel(combinedInput,compressOptions);
		...
		for(const selector of sparseSelectors){
			const bits=[];
			for(let i=0,j=0;1<<j<=selector;)
				if(selector>>j++&1)bits[i++]=j;
		...
		const makeCharClass=(set,toggle)=>{
			const ranges=[];
			for(let i=-1,runStart=i;i<1<<inBits;){
			...
			if(ranges.length>0&&ranges[0][0]==='^')
				1<ranges.length?ranges.push(ranges.unshift()):ranges[0]="\\"+ranges[0];
			return(toggle?'':'^')+ranges.join('')
		};
		// the decoder consists of three loops and its code order doesn't match with the execution order, which is denoted with the preceding number.
		// for the easier manipulation of variables, every variable is named in green alphabet and only replaced with the actual names at the end.
		...
		const sixBit=c=>c^28&&c^63?c|64:c;
/*		// the first few bits are the initial state, loaded immediately at the beginning.
		const stateBytes=[];
		for(let st=state;st>0;st>>=outBits)stateBytes.unshift(st&(1<<outBits)-1);
		let firstLine = `ι='${String.fromCharCode(...stateBytes.map(sixBit))}`;
*/
		let firstLine=`ι='`;
		for(let i=0,l=buf.length;i<l;)firstLine+=String.fromCharCode(...buf.slice(i,i+=8192).map(sixBit));
		firstLine+=`'`;
		...
		const secondLineInit=[
			[`θ`,`1<<${precision+1}`],
			[`ω`,numModels<8?`${JSON.stringify(Array(numModels).fill(0))}`:`Array(${numModels}).fill(0)`],
		...
		let secondLine=exe?
			...
			`);`:
		//fast & small decoder
		`for(${allowFreeVars?`ο=[τ=ρ=λ=Σ=${quotes.length>0?'χ=':''}0],β=[],α=[]`:''};ν=λ<${inputLength};${quotes.length?`ο[λ++]=ν-=${1<<inBits},χ=χ?ν-χ&&χ:${quotes.length>1?`(${quotes.map(q=>`ν==${q}`).join('|')})&&ν`:`ν==${quotes[0]}&&ν`}`:`ο[λ++]=ν-${1<<inBits}`})for(;ν<${1<<inBits};ν+=ν+μ){for(δ of'${selectors.map(i=>i.join('')).join(0)}'.split(ε=0)){φ=0;for(δ of δ)φ=φ*997+(ο[λ-δ]|0)|0;φ=2*π[α[Σ]=${numModels}*(${pow2(contextBits)}-1&φ*997+ν${quotes.length?'+!!χ*129':''})+Σ]+1,β[Σ]=φ=Math.log(φ/(θ-φ)),ε-=φ*ω[Σ++]}for(ε=~-θ/(1+Math.exp(ε))|1;τ<${pow2(28-outBits)};τ=τ*${1<<outBits}|ι.charCodeAt(ρ++)&${(1<<outBits)-1});for(μ=τ%θ<ε,τ=τ%θ+(μ?ε:θ-ε)*(τ>>${precision+1})-!μ*ε;Σ;ω[Σ]+=β[Σ]/${recipLearningRate}*(μ-ε/θ))π[δ=α[--Σ]]+=(μ*${pow2(precision)}-π[δ]<<${29-precision})/((κ[δ]+=κ[δ]<${modelMaxCount})+${modelBaseCount})>>${29-precision}}`;
		...
		if(!preparedJs.code)
		...
		else if(!preparedJs.abbrs.length)
			...
			for(const[,abbr]of preparedJs.abbrs)
				secondLine+=`with(κ.split(\`${escapeCharInTemplate(abbr)}\`))κ=join(shift());`
		}else{
		...
		if(options.allowFreeVars)
			...
			}else secondLine+=`${prefix}${outputVar}${suffix}")(${args})`
		}
		...
		const freeVars=options.allowFreeVars?Object.keys(idMap).filter(v=>!boundVars.includes(v)).map(v=>idMap[v]).sort():[];
	...
	async optimize(level,progress){
		if(typeof level==='function')progress=level,level=0;
		level=level||1;

		const performance=await getPerformanceObject(),cache=new Map; // `${dynamicModels},${numAbbreviations}` -> {preparedText,preparedJs}
		...
		const calculateSize=current=>{
			...
			return new Packed(result).estimateLength(options.zip)
		};
		const reportProgress=async(pass,passRatio,current,currentSize,currentRejected,bestUpdated)=>{
			if(progress&&await progress({pass,passRatio,current,currentSize,currentRejected,best,bestSize,bestUpdated,})===false)throw new Error('search aborted');
		};
		const searchStart=performance.now();
		let best={},bestSize=calculateSize(best);
		await reportProgress('initial',undefined,best,bestSize,false,true);
		const updateBest=current=>{
			const size=calculateSize(current),bestUpdated=size<bestSize;
			if(bestUpdated)best=copy(current),bestSize=size;
		...
		// we don't know anything about f'(x), so we use a modified version of binary search where we pick three points in the range and assume that the smallest is closest to the minimum.
		// the way to pick three points depends on the distribution and affects the search performance.
		const EXP=1,LINEAR=0,search=async(lo,hi,dist,manualValues,score)=>{
			if(level<2){
				for(let i=manualValues.length,l=i;i--;)await score(manualValues[i],i/l);
				return
			}
			...
			let mid=dist^EXP?(x,y)=>x+y>>1:(x,y)=>Math.round(Math.sqrt(x*y));

			const cache=new Map;
			// don't ask me about the ratio computation, this is just an approximation
			const origRange=dist^EXP?hi-lo:Math.log2(hi)-Math.log2(lo)+1;
			const evaluate=async x=>{
				let y=cache.get(x);
				y||cache.set(x,y=await score(x,1-Math.log(dist^EXP?hi-lo:Math.log2(hi)-Math.log2(lo)+1)/Math.log(origRange)));
				return y
			};
			for(let q2=mid(lo,hi);hi-lo>=4;){
				const xx=[lo,mid(lo,q2),q2,mid(q2,hi),hi],yy=[];
				for(const x of xx)yy.push(await evaluate(x));
				let min=0;
				for(let i=0;++i<5;)if(yy[min]>yy[i])min=i;
				if(!min)hi=xx[1],q2=mid(lo,hi);
				else if(min^4)lo=xx[min-1],q2=xx[min],hi=xx[min+1];
				else lo=xx[3],q2=mid(lo,hi)
			}
			// make sure that everything in the final range has been evaluated
			for (let x=lo;++x<hi;)await evaluate(x);
		};
		...
		best.dynamicModels===this.options.dynamicModels&&delete best.dynamicModels;
		...
		best.numAbbreviations===this.options.numAbbreviations&&delete best.numAbbreviations;

		// optimize sparseSelectors by simulated annealing
		let current=this.options.sparseSelectors.slice(),currentSize=bestSize,temperature=1;
		const taboo=new Map,targetTemperature=level<2?.9:.1;
		for(;temperature>targetTemperature;temperature*=.99){
			const next=current.slice();
			let added;
			while(next.includes(added=0|Math.random()*AUTO_SELECTOR_LIMIT));
			next[0|Math.random()*next.length]=added;
			next.sort((a,b)=>a-b);
			...
			await reportProgress("sparseSelectors",Math.log(temperature)/Math.log(targetTemperature),{...best,sparseSelectors:next},nextSize,rejected,bestUpdated);
			if(!rejected)current = next,currentSize = nextSize
		}
		// optimize precision
		...
		best.recipLearningRate===this.options.recipLearningRate&&delete best.recipLearningRate;
...
/*	the first line is mostly 6-bit code, but there are additional characters so one of the 6-bit code literal has to be encoded in 7 bits.
	if the original data is N bytes the first line will contain about 4/3 N codes, 1/64 of which will have to use one additional bit.
