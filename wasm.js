const getContextItemShift=({precision,modelMaxCount})=>
	Math.max(precision>8&&1+(precision>16),modelMaxCount<128?0:modelMaxCount<32768?1:2);

// this is a WebAssembly version of compressWithModel with a DefaultModel, minus AnsEncoder (which should be run independently with returned probabilities). this should return an *identical* result to the JS version.
const makeDefaultModelRunner=contextItemShift=>{
	...
	const signed=v=>{for(var a=0,b=[];63<v||v<-64;v>>=7)b[a++]=127&v|128;return b[a++]=127&v,b};
	const binary64=v=>{const a=new Uint8Array(8);new DataView(a.buffer).setFloat64(0,v,!0);return[...a]};
	const ensure_i32=a=>{if(void 0===a)throw"i32 argument missing";return Array.isArray(a)?a:i32(a)};
	const ensure_i64=a=>{if(void 0===a)throw"i64 argument missing";return Array.isArray(a)?a:i64(a)};
	const ensure_f64=a=>{if(void 0===a)throw"f64 argument missing";return Array.isArray(a)?a:f64(a)};
	const I32=127,I64=126,F32=125,F64=124,END=11;
	const loop=(...body)=>[3,64].concat(...body,[END]);
	const if_=(cond,...body)=>[...cond,4,64].concat(...body,[END]);
	const if_else=(cond,...thenBody)=>(...elseBody)=>[...cond,4,64].concat(...thenBody,[5],...elseBody,[END]);
	const call=(f,...args)=>f<0?[]:[].concat(...args,[16,~f]);
	const select=(cond,a,b)=>[...a,...b,...cond,27];
	const br_if=(depth,v)=>[...v,13,~depth];
	...
	const memory_fill=(addr,v,n)=>[...ensure_i32(addr),...ensure_i32(v),...ensure_i32(n),0xfc,0x0b,0];
	...
	// unlike JS version, counts and predictions are spatially local to each other. this however means that they should be using the same type for alignments.
	const M_CONTEXTS=0x700;
/*
	inputs and outputs are variable, so we refer them with an offset to M_CONTEXTS. the "pregap" before the input is required because contexts for first bytes are padded with zeroes if they precede the first byte.
...
*/
	...
	// i32 arguments
	const INBITS=localCount++,INPUTLEN=localCount++,CONTEXTBITS=localCount++,NUMSELECTORS=localCount++,PRECISION=localCount++,QUOTE=localCount++,MODELMAXCOUNT=localCount++,MARK=localCount++,NI32ARGS=localCount // modelQuotes in the argument position (-1 or 0)

	// f64 arguments
	,MODELBASECOUNT=localCount++,RECIPLEARNINGRATE=localCount++,NF64ARGS=localCount-NI32ARGS

	// local i32 variables
	,NI32LOCALS=11,I32LOCALS=localCount;
	localCount+=NI32LOCALS;
	let nextI32Local=0;
	const withI32Locals=f=>{
		const locals=[],l=f.length;
		for(let i=0;i<l;)locals[i++]=I32LOCALS+nextI32Local++;
		if(nextI32Local>NI32LOCALS)throw'i32 local overflow';
		f=f(...locals);nextI32Local-=l;
		return f
	};
	// local i64 variables
	const NI64LOCALS=1,I64LOCALS=localCount;
	localCount+=NI64LOCALS;
	let nextI64Local=0;
	const withI64Locals=f=>{
		const locals=[],l=f.length;
		for(let i=0;i<l;)locals[i++]=I64LOCALS+nextI64Local++;
		if(nextI64Local>NI64LOCALS)throw'i64 local overflow';
		f=f(...locals);nextI64Local-=l;
		return f
	};
	// local f64 variables
	const NF64LOCALS=2,F64LOCALS=localCount;
	localCount+=NF64LOCALS;
	let nextF64Local=0;
	const withF64Locals=f=>{
		const locals=[],l=f.length;
		for(let i=0;i<l;)locals[i++]=F64LOCALS+nextF64Local++;
		if(nextF64Local>NF64LOCALS)throw'f64 local overflow';
		f=f(...locals);nextF64Local-=l;
		return f
	};
	const body=[
		// local variables
		vec([~NI32LOCALS,I32],[~NI64LOCALS,I64],[~NF64LOCALS,F64],),
...
*/
			...
			// do{
			loop(...withI32Locals((BYTE,BITIDX,SELIDX)=>[
...
*/
								...
								// do{
								set(SELIDX,0),
								loop(...withI32Locals(CONTEXTHASH=>[
/*
...
*/
											...
											storeContextItem(M_CONTEXTS,add(get(CONTEXTHASH),2<<contextItemShift),shl(1,sub(get(PRECISION),1))),
										// }
										),
										...withF64Locals(PROB=>[
							...
							// do{
							set(SELIDX,0),
							loop(...withI32Locals((CONTEXTHASH,COUNT)=>[
/*
									M_CONTEXTS[
...
*/
	...
	const resolve=data=>{
		const bytes=[];
		const u32=v=>{
			for(;v>=0x80;v>>=7)bytes.push(v&0x7f|0x80);
			bytes.push(v)
		};
		for(const x of data){
	...
	const instance=new WebAssembly.Instance(new WebAssembly.Module(new Uint8Array(resolve(data))),{Math,console});
	...
	return(input,{inBits,contextBits,sparseSelectors,precision,modelQuotes,modelMaxCount,modelRecipBaseCount,recipLearningRate})=>{
		...
		const M_PREGAP=M_CONTEXTS+(3*sparseSelectors.length<<contextBits+contextItemShift);
		const M_INPUT=M_PREGAP+32,M_OUTPUT=M_INPUT+(input.length+3&-4),M_END=M_OUTPUT+input.length*inBits*4;

		// grow the memory if needed
		const lastMemorySize=memory.buffer.byteLength;
		M_END>lastMemorySize&&memory['grow']((M_END-memory.buffer.byteLength+65535)>>16);

		// clear the memory if needed (see DirectContextModel about the mark)
		if(++mark===maxMark)
			mark=1,new Uint8Array(memory.buffer).fill(0);
		else{
/*
...
*/
			...
			M_CONTEXTS_END<dirtyEnd&&new Uint8Array(memory.buffer).fill(0,M_CONTEXTS_END,dirtyEnd)
		}
		...
		for(let i=0;i<sparseSelectors.length;++i){
			...
			for(let j=0;j<15;)
				if(sel>>j++&1)
					...
					indicesHi=indicesHi<<4|indicesLo>>>28,indicesLo=indicesLo<<4|j;
			selectorView[i*2]=indicesLo;
			selectorView[i*2+1]=indicesHi;
		}
		new Uint8Array(memory.buffer,M_INPUT).set(input);
		let quotesSeenBits=input.length&&run(inBits,input.length,contextBits,sparseSelectors.length,precision,modelQuotes?0:-1,modelMaxCount,mark,1/modelRecipBaseCount,recipLearningRate);
		const quotesSeen=new Set;
		1&quotesSeenBits&&quotesSeen.add(34),2&quotesSeenBits&&quotesSeen.add(39),4&quotesSeenBits&&quotesSeen.add(96);
		return{predictions:new Uint32Array(memory.buffer,M_OUTPUT,input.length*inBits),quotesSeen}
	}
}